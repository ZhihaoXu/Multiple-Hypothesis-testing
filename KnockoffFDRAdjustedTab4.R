...
simulate_kf = function(N,delta,q=0.2,size=100,type="Oracle"){
  ...
  for (k in 1:size){
    ...
    info<- sprintf("Finished %d%%", round(k*100/size))
    setTkProgressBar(pb, k*100/size, sprintf("Process (%s)", info),info)
  }
  close(pb)
...
# Table 4 (It may take long time to run the following code)
delta = c(rep(0.5,10))
