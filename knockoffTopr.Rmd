...
simulate_topr_Dummy = function(topr,mu,Sigma=0,r=10,a=46.55){
  ...
  if (length(Sigma)==1){
    ...
    for (j in 1:topr$StopT){
      # x_ = rep(0,topr$N)
      # x_[topr$r.index] = topr$data[,j][topr$r.index]
...
size=100
for (k in 1:size){
  topr = simulate_topr(n=sig,mug=0.3,muw=0.5,Sigma=0,N=1000,r=100,a=200)
...
size=100
for (k in 1:size){
  topr = simulate_topr(n=sig,mug=0.8,muw=0.5,Sigma=0,N=1000,r=10,a=35)
  rej = knockoff1(topr,alpha=0.3)
