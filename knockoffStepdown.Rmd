...
# 
# simulate_topr_GlobalNull_B_CI = function(topr,mu,rejIndex,tindex,r=10,B = 500,a = 46.55){
#   indicator = c()
#   for (i in 1:B){
#     Wr = simulate_topr_GlobalNull(topr$StopT,mu, r = 10, a = 46.55)
#     if (tindex %in% Wr$r.index == FALSE){
#       i = i - 1
#       next
#     }
#     W0 = Wr$W[tindex]
...
# t = Stopping time T, K = Number of censor
simulate_topr_CI = function(mu,r=10,a=46.55){
  W = rep(0,100)
  m = 0
  data = c()
  while (TRUE){
...
knockoffAcp = function(topr){
  ...
  #     lb = 0
  #   } else {
  #     lb = CI1(topr,tindex=i,mu,rejIndex)
  #   }
  #   LB = c(LB,lb)
  # }
  # p = cbind(p,LB)
  # p = cbind(p,apply(topr$data[topr$r.index,],1,mean))
  # colnames(p) = c("index","p-value","p-valueBH","CILowerBond","muHat")
...
```{r}
CI = function(topr,tindex){
  theta = 100
  a = mean(topr$data[tindex,]) ##<0
  fa = computeF(topr,theta=a,mu=mu,tindex)
  b = a - 2*1/sqrt(topr$StopT)
  fb = computeF(topr,theta=b,mu=mu,tindex)
  while (TRUE){
    if (fb<=0){
  ...
  while ((abs(theta-oldTheta)>=0.01)&(k<=5)){
    f0 = computeF(topr,theta=theta,mu=mu,tindex)
    if (f0<0){
      a = theta
      fa = f0
    }else{
...
```{r}
topr = simulate_topr(n=10)
p = knockoffAcpCI(topr)
```
